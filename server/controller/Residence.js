...
exports.create = (req, res) => {
    ...
    const picture = `http://localhost:5000/residence/${req.files.picture.name}`;
...
exports.download = (req, res) => {
    ...
    Record.findOne({ $and:[{name: req.body.name},{forReason:"Residence Certificate"}] }, (err, user) => {
        if (!user) {
            return res.status(400).json({
                error: `No Payment Record found with name ${req.body.name}!`
            });
        } else {
